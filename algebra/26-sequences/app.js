...
const first=num(2,4),difference=num(3,3),count=Math.max(1,Math.trunc(num(4,8))); const terms=Array.from({length:count},(_,i)=>first+i*difference); console.log(`Sequence: ${terms.join(', ')}`); console.log(`Common difference: ${difference}`); console.log(`Term ${count}: ${fmt(terms.at(-1))}`);
