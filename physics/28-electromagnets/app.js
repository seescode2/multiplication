...
const current = numberArg(2, 2, "current");
const turns = integerArg(3, 20, "turns", 1, 1000);
const strength = current * turns;
const poles = current >= 0 ? "N ===== S" : "S ===== N";
console.log(`${turns} turns carrying ${current} A: ${poles}`);
console.log(`Relative field strength = ${Math.abs(strength).toFixed(1)}; direction sign = ${Math.sign(strength) || 0}.`);
